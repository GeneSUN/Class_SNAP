...
echo "RES: Starting===" 
echo $(date) 
/usr/apps/vmas/spark-3.2.0-bin-hadoop3.2/bin/spark-submit --master spark://njbbepapa1.nss.vzwnet.com:7077 --conf "spark.sql.session.timeZone=UTC" --conf "spark.driver.maxResultSize=2g" --conf "spark.dynamicAllocation.enabled=false" --num-executors 150 --executor-cores 2 --total-executor-cores 300 --executor-memory 2g --driver-memory 16g --packages org.apache.spark:spark-avro_2.12:3.2.0,io.streamnative.connectors:pulsar-spark-connector_2.12:3.2.0.2 /usr/apps/vmas/script/ZS/SNAP/class_SNAP.py> /usr/apps/vmas/script/ZS/SNAP/class_SNAP.log
